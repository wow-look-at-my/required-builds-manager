...
A Cloudflare Worker that acts as a GitHub App webhook handler. It listens for `status`, `check_run`, and `workflow_run` events, aggregates all build states for a commit, and publishes a single "all-builds" combined **check run** using a low-water-mark algorithm (failure > pending > success). The check run's `output.title` is a running count (e.g. `2/3 builds passed`, or `1/3 builds failed`) that updates as builds finish, and its Markdown `output.summary` breaks every build into Failed/In progress/Passed sections — each build a link to its check — naming which failed and why (on failure the passing builds are omitted to keep focus on what's broken). A check run is used instead of a commit status precisely because the status `description` is capped at ~140 chars while a check run's `output.summary` holds a full Markdown breakdown. Listening for `workflow_run` lets it catch `startup_failure` (e.g. invalid workflow YAML), which produces no statuses or check runs and would otherwise be invisible.
...
├── github.ts      # GitHub API client: listStatuses, listCheckRuns, listWorkflowRuns, getWorkflowJob (job steps), publishCheckRun (create-or-update-in-place; sets details_url to the commit's GitHub Checks page; attaches HTTP status to thrown errors) (paginated)
...
7. Aggregate: fetch all statuses + check-runs + workflow-runs for the SHA, deduplicate by context/name, filter out ignored patterns, collect each build's name/kind/state/detail/url/timing, enrich each failed or in-progress Actions job with its individual steps, fold in the triggering event **unless it's pending** (a pending incoming is dropped — the deduped listing is already authoritative for every build's success/pending state, so trusting one only risks wedging `all-builds` on "in progress"; a failure or success incoming is still folded for correctness under list-endpoint lag), compute the combined state, and render the Markdown title + summary
8. Publish the combined check run using the configured name (`output.title` is a running count like `2/3 builds passed`; `output.summary` is the full per-build breakdown with links), routed through the per-commit Durable Object (`publishViaCoordinator` → `CheckRunPublisher`). `publishCheckRun` first looks up our existing run for the SHA (by name + our `app.id`) and PATCHes it in place, only POSTing a new one if none exists — so a commit shows a single `all-builds` entry that changes state, not a stack of duplicates. A `pending` aggregate is an `in_progress` run (no conclusion); success/failure/error are `completed` runs
...
- **Output is a check run, not a status**: The combined result is published via `publishCheckRun` so the `output.summary` Markdown can carry a full per-build breakdown — a commit-status `description` is capped at ~140 chars. `aggregate.ts` collects each build's name/kind/state/detail/url/timing and renders: a `title` that is a running count (`{passed}/{total} builds passed`, or `{failed}/{total} builds failed` on any failure — it updates as builds finish, since every event re-aggregates) and a `summary` grouping builds into Failed/In progress/Passed. Each build is a Markdown link to its check (`[name](<url>)`); a failed or in-progress Actions job also nests its individual steps (which step failed / is running). On failure the Passed section is dropped so the focus is on what broke. A `Total time` line (earliest start → latest finish) is appended when builds carry timing (check runs and workflow runs do; statuses don't). The check run's `started_at` is also set to that earliest build start, so GitHub's own "Successful in Xs" line renders the total CI wall-clock rather than our worker's (near-instant) processing time. Names/details are escaped (not wrapped in code) so arbitrary text can't break the Markdown; underscores are left unescaped since GFM doesn't emphasize intra-word `_`. `publishCheckRun` also sets the run's `details_url` (the check's "Details" link) to the commit's GitHub Checks page (`https://github.com/{owner}/{repo}/commit/{sha}/checks`) — the native view listing every build's result for the commit. Without this, GitHub defaults the link to the App's configured homepage URL (the bare worker domain), which serves nothing useful on a GET.
