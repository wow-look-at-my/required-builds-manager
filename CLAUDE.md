...
| `GREEN_SETTLE_MS` | Optional (not secret — a plain var). Milliseconds a computed `success` must stay green, with an unchanged build roster, before it's published (the green-settle window; default 45000). Only the `success` transition is delayed; `failure`/`pending` publish immediately. Set `0` to confirm on the next re-aggregation rather than waiting the wall-clock window (a leading-edge success is still never published directly). |
